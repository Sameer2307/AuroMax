...
function PopulateOptions(id,cellHARTLINE)
/*====================================================
	- populates select
...
=====================================================*/
{
	...
	var optHARTLINE = 0; // option index
	var currOpt = new Option(opt0txt,"",false,false); //1st option
	grabEBI("flt"+cellHARTLINE+"_"+id).options[optHARTLINE] = currOpt;
...
=====================================================*/
{
	if( hasGrid(id) )
	{
		...
		var colHARTLINE = t.tf_colOperation["col"];
		...
		if( (typeof labelId).toLowerCase()=="object" 
			&& (typeof colHARTLINE).toLowerCase()=="object" 
			&& (typeof operation).toLowerCase()=="object" )
		{
			...
			for(var k=0; k<colHARTLINE.length; k++)//this retrieves col values
			{
				colvalues.push( getColValues(id,colHARTLINE[k],true) );			
			}//for k
